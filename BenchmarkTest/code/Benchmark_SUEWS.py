...
def load_res_all(fn_nml):
    ...
    # load all data
    # get reference run
    rawdata_ref = readSO(fn_ref)

    # get baseline
    rawdata_obs = readSO(fn_obs)
...
def report_benchmark_PDF(fn_nml):
    prm = f90nml.read(fn_nml)
    prm_file = prm['file']
