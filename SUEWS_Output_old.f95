...
!Last change by HCW 18 Nov 2014
...
FileOut=trim(FileOutputPath)//trim(FileCode)//trim(adjustl(grstr2))//'_'//trim(adjustl(yrstr2))// &
	'_'//trim(adjustl(str2))//'.txt'
!FileOut=trim(FileOutputPath)//trim(FileCode)//'_'//trim(adjustl(str2))//'.txt'
NARPOut=trim(FileOutputPath)//trim(FileCode)//trim(adjustl(grstr2))//'_'//trim(adjustl(yrstr2))// &
	'_NARPOut.txt'
SnowOut=trim(FileOutputPath)//trim(FileCode)//trim(adjustl(grstr2))//'_'//trim(adjustl(yrstr2))// &
  	'_SnowOut.txt'
...
if (NARPOutput==1) then
   if(iv == 1 ) then   
      ...
      open(7,file=NARPOut,position='append')
   endif
endif
...
if (snowUse==1) then   
   if(iv == 1 ) then   
      ...
      open(8,file=SnowOut,position='append')
   endif
endif
...
!do i=1,irMax
do i=1,(irMax-1)
   if(dataOut1(i,2,Gridiv) /= dataOut1((i+1),2,Gridiv)) then
      write(lfnoutC,301) int(dataOut1(i,1,Gridiv)),int(dataOut1(i,2,Gridiv)),(dataOut1(i,is,Gridiv),is = 3,62)   
   endif
   if (NARPOutput==1)  write(7,117) int(dataOut2(i,1,Gridiv)),(dataOut2(i,is,Gridiv),is = 2,30)
   if (snowUse==1)     write(8,118) int(dataOut3(i,1,Gridiv)),int(dataOut1(i,2,Gridiv)),(dataOut3(i,is,Gridiv),is = 3,106)
   !if (SOLWEIGpoi_out==1) write(9,119) int(dataOutSOL(i,1)),(dataOutSOL(i,is),is = 2,28) 
enddo

do i=irMax,irMax
   if(dataOut1(i,2,Gridiv) == dataOut1((i-11),2,Gridiv)) then
      write(lfnoutC,301) int(dataOut1(i,1,Gridiv)),int(dataOut1(i,2,Gridiv)),(dataOut1(i,is,Gridiv),is = 3,62)
   endif
   if (NARPOutput==1)  write(7,117) int(dataOut2(i,1,Gridiv)),(dataOut2(i,is,Gridiv),is = 2,30)
   if (snowUse==1)     write(8,118) int(dataOut3(i,1,Gridiv)),int(dataOut1(i,2,Gridiv)),(dataOut3(i,is,Gridiv),is = 3,106)
