...
choosegridsbasic = plotnml['plot']['choosegridsbasic']
chooseyearsbasic = plotnml['plot']['chooseyearsbasic']
plotmonthlystat = plotnml['plot']['plotmonthlystat']
choosegridsstat = plotnml['plot']['choosegridsstat']
chooseyearsstat = plotnml['plot']['chooseyearsstat']

if plotbasic == 1:
    if choosegridsbasic:
        gridcode = choosegridsbasic

    if chooseyearsbasic:
        YYYY = chooseyearsbasic

    suews_out = wf + fileoutputpath[1:] + filecode + gridcode + '_' + str(YYYY) + '_60.txt'

    if chooseyearsbasic or choosegridsbasic:
        suews_1hour = np.loadtxt(suews_out, skiprows=1)

    pl.plotbasic(suews_1hour, met_old)  # met_old should change if multiple input is used

if plotmonthlystat == 1:
    if choosegridsstat:
        gridcode = choosegridsstat

    if chooseyearsstat:
        YYYY = chooseyearsstat
